...
(function vCardFormatter() {
  ...
  module.exports = {
    ...
    getFormattedString: function (vCard) {
      ...
      if (vCard.birthday) {
        formattedVCardString +=
          "BDAY:" + moment(vCard.birthday).format("YYYYMMDD") + nl();
      }

      if (vCard.anniversary) {
        formattedVCardString +=
          "ANNIVERSARY:" + moment(vCard.anniversary).format("YYYYMMDD") + nl();
      }

      if (vCard.email) {
        if (majorVersion >= 4) {
          formattedVCardString +=
            "EMAIL" + encodingPrefix + ";type=HOME:" + e(vCard.email) + nl();
      ...
      if (vCard.pagerPhone) {
        if (majorVersion >= 4) {
          formattedVCardString +=
            'TEL;VALUE=uri;TYPE="pager,cell":tel:' + e(vCard.pagerPhone) + nl();
        } else {
          formattedVCardString +=
            "TEL;TYPE=PAGER:" + e(vCard.pagerPhone) + nl();
        }
      }
      ...
      if (vCard.homePhone) {
        if (majorVersion >= 4) {
          formattedVCardString +=
            'TEL;VALUE=uri;TYPE="voice,home":tel:' + e(vCard.homePhone) + nl();
        } else {
          formattedVCardString +=
            "TEL;TYPE=HOME,VOICE:" + e(vCard.homePhone) + nl();
        }
      }
      ...
      if (vCard.homeFax) {
        if (majorVersion >= 4) {
          formattedVCardString +=
            'TEL;VALUE=uri;TYPE="fax,home":tel:' + e(vCard.homeFax) + nl();
        } else {
          formattedVCardString +=
            "TEL;TYPE=HOME,FAX:" + e(vCard.homeFax) + nl();
        }
      }
      ...
      if (vCard.workFax) {
        if (majorVersion >= 4) {
          formattedVCardString +=
            'TEL;VALUE=uri;TYPE="fax,work":tel:' + e(vCard.workFax) + nl();
        } else {
          formattedVCardString +=
            "TEL;TYPE=WORK,FAX:" + e(vCard.workFax) + nl();
        }
      }
